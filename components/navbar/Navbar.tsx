    ...
    <nav className='flex justify-between items-center w-full bg-[#323232] px-5.75 py-8 '>
      ...
      <div className='flex justify-between items-center gap-16'>
        ...
        {NavbarLinks.map((link) => (
          <a key={link.name} href={`/${link.href}`} className={`relative uppercase text-2xl font-dmSans tracking-[-0.264px] font-bold text-center ${pathname === `/${link.href}` ? "underline decoration-[#F3FF85] decoration-3 underline-offset-8" : ""} max-sm:hidden`}>
            {link.name}
